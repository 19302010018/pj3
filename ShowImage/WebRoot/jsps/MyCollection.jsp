			...
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="navbar-menu">
					<ul class="sina-menu" data-in="fadeInLeft" data-out="fadeInOut">
						...
						<c:if test="${CollectionType == 2 }">
							<li class="scroll"><a href="<c:url value='/GeoServlet?method=getCountry'/>">${fname }的收藏</a></li>
						</c:if>
					</ul>
				</div><!-- /.navbar-collapse -->
			</div><!-- .container -->
				...
				<div data-sjsel="funny">
					<div class="card">
						...
						<div class="card-infos">
							...
							<c:choose>
		                		<c:when test="${CollectionType == 2 }">	
			                		<p class="card__text">
										${myImage.description }
									</p>
									<c:choose>
										<c:when test="${myImage.isCollection==1 }">
										<div style="float:right">
												<a href="<c:url value='/MyImageServlet?method=deleteCollection&uid=${sessionScope.sessionUser.uid }&imageId=${myImage.imageId }'/>">
													<Button style="margin-left:20px" class="btn btn-primary">取消收藏</Button>
												</a>
										</div>
										</c:when>
										<c:otherwise>
											<div style="float:right">
												<a href="<c:url value='/MyImageServlet?method=addCollection&uid=${sessionScope.sessionUser.uid }&imageId=${myImage.imageId }'/>">
													<Button style="margin-left:20px" class="btn btn-primary">收藏</Button>
												</a>
											</div>
										</c:otherwise>
									</c:choose>
